...
def data_loader(file):
    ...
    with codecs.open(file1, 'r') as f:
        ...
        for line in content:
            ...
            triple_list.append([h_, r_, t_,p_])
    ...
    with codecs.open(file4, 'r') as f:
        ...
        for line in content:
            ...
            relation_set.add(r_)
    if "1084" in entity_set:
        print("1084 is in the set")
    else:
        print("1084 is not in the set")
    #pdb.set_trace()
...
class TransH():
    def __init__(self, entity_set, relation_set, triple_list, embedding_dim = 50, lr = 0.01, margin = 1.0, C = 1.0,negen=1e-4,negep=1e-13):
    ...
    def training_run(self, epochs=100, times = 2 ,nbatches = 50):
        ...
        for epoch in range(epochs):
            ...
            loss_ls.append(self.loss/len(self.triples))
    ...
    def update_triple_embedding(self, Tbatch):
        for correct_sample, corrupted_sample in Tbatch:
            ...
            correct_p = torch.tensor(float(correct_sample[3]))
            corrupted_p = torch.tensor(float(corrupted_sample[3]))
            loss = F.relu(self.margin + correct_distance - corrupted_distance)
            posProb_loss=torch.abs(1.0/self.t*torch.log(torch.reciprocal(correct_p))-correct_distance)
            negProb_loss=torch.abs(1.0/self.t*torch.log(torch.reciprocal(corrupted_p))-corrupted_distance)
            new_loss=torch.exp(1.0/self.k*loss)*(posProb_loss+negProb_loss)
            new_loss.backward()
            self.loss += new_loss.item()
            #loss.backward()
            #self.loss += loss.item()
            opt1.step()
            opt2.step()
...
if __name__ == '__main__':
    ...
    transH.training_run(epochs=40,times=1,nbatches=400)#times表示第几次训练，跟最后生成的参数文件名有关